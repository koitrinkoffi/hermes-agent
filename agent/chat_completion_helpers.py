...
def cleanup_task_resources(agent, task_id: str) -> None:
    ...
    Skips ``cleanup_browser`` when the session is a persistent local profile
    (hermes-mods) or in headed mode (upstream) so the browser survives
    between turns either way. The inactivity reaper in
    ...
    try:
        ...
        #
        # Also skip in headed mode (upstream): a visible browser window
        # shouldn't flash closed after every turn either. OR the two
        # conditions together — either one is sufficient to keep the browser.
        persistent = _ra().is_persistent_browser_session(task_id)
        headed = False
        try:
        ...
        if persistent or headed:
            if agent.verbose_logging:
                reason = "persistent" if persistent else "headed"
                logging.debug(
                    f"Skipping per-turn cleanup_browser for {reason} "
