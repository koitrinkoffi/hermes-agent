...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Local mod: the override applies to whichever variant upstream selects.
    stable_parts.append(
        _prompt_override("help_guidance", HERMES_AGENT_HELP_GUIDANCE_NO_SKILLS)
    )
    ...
    if "memory" in agent.valid_tool_names:
        if _mem_enabled:
            ...
            tool_guidance.append(
                _prompt_override("user_profile_guidance", USER_PROFILE_GUIDANCE)
            )
    ...
    if agent.valid_tool_names:
        ...
        if _inject:
            ...
            if "gemini" in _model_lower or "gemma" in _model_lower:
                stable_parts.append(
                    _prompt_override("google_guidance", GOOGLE_MODEL_OPERATIONAL_GUIDANCE)
                )
    ...
    if agent.valid_tool_names:
        ...
        if _exec_inject:
            ...
            stable_parts.append(
                _prompt_override(
                    "openai_guidance", execution_guidance_text(agent.valid_tool_names)
                )
            )
    ...
    if _has_skill_view and "- hermes-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = _prompt_override(
            "help_guidance", HERMES_AGENT_HELP_GUIDANCE
        )
