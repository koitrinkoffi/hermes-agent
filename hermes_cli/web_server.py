...
def _is_accepted_host(
    ...
    - Any host in ``extra_allowed`` (local mod: operator-configured
      ``dashboard.extra_allowed_hosts``, e.g. a `tailscale serve` name)
      when bound to loopback.  Deliberately NOT folded into
      ``trusted_public_hosts``: that set also drives
      ``should_require_dashboard_auth``, so folding it in would switch the
      auth gate on for an install that has no auth provider configured.
    """
...
async def host_header_middleware(request: Request, call_next):
    ...
    if bound_host:
        ...
        if not _is_accepted_host(
            host_header, bound_host, trusted_public_hosts, extra_allowed
        ):
            return JSONResponse(
                status_code=400,
                content={
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    if not _is_accepted_host(
        parsed.netloc, bound_host, trusted_public_hosts, extra_allowed
    ):
        return f"origin_mismatch origin={origin} bound={bound_host}"
    return None
