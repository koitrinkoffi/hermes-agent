...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        + restrictions_rule +
        "- Child model IS selectable per call via 'model' (top-level, or per "
        "task inside 'tasks'). Omit it and children inherit delegation.model "
        "from config.yaml, else the parent model and its fallback chain. "
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    if "model" in tasks_items["properties"]:
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            # Local mod: per-call child model + named agent types. Kept
            # top-level (upstream folded its own params into `tasks`) because
            # both apply to the whole call when set here; the per-task
            # equivalents inside `tasks.items` override them.
            "model": {
                "type": "string",
                "description": "(rebuilt at get_definitions() time)",
            },
